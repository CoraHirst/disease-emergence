...
It is helpful to see how $P_Emergence$ depends on the $R_0$ of the initial spillover infection and the number of mutations required for adaptation. Below, we plot curves for the probability of emergence of a variant spilling over with some inital $R_0$, depending on the increase in fitness gained from a mutation (`r paste("m =", ms)` gives the number of mutations required to reach an $R_0 = 1$ from an $R_0$ of 0).
...
### We will be creating two different sets of curves, correlating with different mutation rates. I want to store both data frames in a list. 
...
for(k in 1:length(mus)){ #creating a seperate dataframe, probs.emergence, for each mu
  ...
  #add data frame to storage list 
  emergence_curves[[k]] = probs.emergence %>% mutate("mu" = mu)
...
# plot
plot = ggplot() + 
  geom_line(data = emergence_curves[[1]], aes(x = R0, y = P.Emergence, group = m, col = m, linetype = as.factor(mu))) + #plot p emergence as function of R0_1 for each m
...
for(i in 2:length(emergence_curves)){
  plot = plot +  geom_line(data = emergence_curves[[i]], aes(x = R0, y = P.Emergence, group = m, col = m, linetype = as.factor(mu)), lwd = 1.25)  #plot p emergence as function of R0_1 for each m
}
...
Below, we plot the dependence of $P_{emergence}$ on $\mu$ and $R0_1$, given different combinations of $m$ and $f_v$. 
...
# plot
plot = ggplot() + 
  geom_line(data = probs.emergence, aes(x = R0, y = P.Emergence, group = m)) + #plot p emergence as function of R0_1 for each 
...
# plot
plot = ggplot() + 
  geom_line(data = probs.emergence, aes(x = R0, y = P.Emergence, group = m), col = "white") + #plot p emergence as function of R0_1 for each m
  geom_line(data = probs.emergence %>% filter(m == ms[1], R0 > 1-(1/length(ms)), R0 <= 1), aes(x = R0, y = P.Emergence, group = m), col = "red", size = 2) +
  geom_line(data = probs.emergence %>% filter(m == ms[2], R0 > 1-2*(1/length(ms)), R0 <= 1-(1/length(ms))), aes(x = R0, y = P.Emergence, group = m), col = "red", size = 2) +
  geom_line(data = probs.emergence %>% filter(m == ms[3], R0 > 1-3*(1/length(ms)), R0 <= 1-2*(1/length(ms))), aes(x = R0, y = P.Emergence, group = m), col = "red", size = 2) +
  geom_line(data = probs.emergence %>% filter(m == ms[4], R0 > 1-4*(1/length(ms)), R0 <= 1-3*(1/length(ms))), aes(x = R0, y = P.Emergence, group = m), col = "red", size = 2) +
  ### adding horizontal dashed lines in powerpoint
  theme_bw() + #white background with grey gridlines
  labs(x = "R0 of first infection", y = "Probability of emergence", title = "probability of emergence with m phenotypes") + #axis and title labels
  scale_x_continuous(breaks = seq(0, 1, by = 0.1)) +
  scale_y_continuous(trans = "log10", limits = c(10^-15,1)) 
...
R0_2 = 1 #R0 at which human-human transmission is achieved/end of interval
Rfinal = 1000 #R0 of final phenotype
m = 5 #how many types? m-1 mutations required, meaning m-2 intermediate types
mu = 10^-3 #mutation rate
  ...
  ## simulate
    ...
    for(i in 1:length(R0_1s)){
      probs.emergence[i] = pEmergence_fixed(m = m, mu = mu, R0_1 = R0_1s[i], R0_2 = R0_2, Rfinal = Rfinal) #run simulation for each m,R0_1 combination
    }
  
  ## format matrix for plotting
  probs.emergence = cbind(R0_1s, probs.emergence) %>% as.data.frame() #format as dataframe with column denoting the R0_1 used to generate the probabilities row-wise and column for each m used to generate the probabilities column-wise
  colnames(probs.emergence) = c("R0", "P.Emergence") #name the columns for easy referencing
...
# plot
plot = ggplot() + 
  geom_line(data = probs.emergence, aes(x = R0, y = P.Emergence)) + #plot p emergence as function of R0_1 for each m
  theme_bw() + #white background with grey gridlines
  labs(x = "R0 of introduced pathogen", y = "Probability of Emergence", title = "probability of emergence with m phenotypes") + #axis and title labels
  scale_x_continuous(breaks = seq(0, 1, by = 0.1)) +
  scale_y_continuous(trans = "log10", limits = c(10^-12,1))  #log10 y scale
...
# parameters
mus = 10^seq(-6,-3, by = 0.1) #mutation rate
R0_1s = seq(0.01,1.0, by = 0.01) #wt R0s to test
R0_2 = 1.0 #emergence R_0
Rfinal = 1000 #final phenotype R_0
m = 5 #how many types? m-1 mutations required, meaning m-2 intermediate types
...
# storage matrix 
  ...
  for(i in 1:length(R0_1s)){
    for(j in 1:length(mus)){
    ## wild-type probability
      P.E = pEmergence_fixed_evenlydispersed(m = m, mu = mus[j], R0_1 = R0_1s[i], R0_2 = R0_2, Rfinal = Rfinal) #calculated emergence probability for each combinations
